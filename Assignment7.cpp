...
IMPLEMENT_GEOX_CLASS( Assignment7, 0)
{
	...
	ADD_NOARGS_METHOD(Assignment7::DrawStreamLines)
...
Assignment7::Assignment7()
{
	...
	//fileName = "C:\\Program Files\\GeoX\\experiments\\Visualization\\Assignment7\\Data\\ANoise2CT4.am";	//Anders
	//fileName = "";																					//Jim
...
Vector2f Assignment7::EulerIntegration(Vector2f pos) {
	return pos + field.sample(pos[0],pos[1])*EulerStepSize;
}

Vector2f Assignment7::EulerIntegration(Vector2f pos, bool forwards) {
	float dt;
	if(forwards) {
		dt = EulerStepSize;
	} else {
...
void Assignment7::DrawStreamLines()
{
	//viewer->clear();
	if(!readField){
		ReadFieldFromFile();
...
vector<Vector2f> Assignment7::GenerateStreamLineEquidistant(Vector2f startPoint, float segmentLength) {
...
vector<Vector2f> Assignment7::GenerateStreamLineEquidistantLong(Vector2f startPoint, float segmentLength) {
